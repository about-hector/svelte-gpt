...
<script lang="ts">
	...
	export let shouldScaleBackground: $$Props['shouldScaleBackground'] = defaults?.shouldScaleBackground;
        export let overlayRef: $$Props['overlayRef'] = defaults?.overlayRef;

    export let drawerRef: $$Props['drawerRef'] = defaults?.drawerRef;
...
const rootCtx = setContext({
	...
	// TODO: figure out why
	$: rootCtx.update((v) => ({ ...v, open, modal, dismissible, shouldScaleBackground, overlayRef, drawerRef }));
</script>
